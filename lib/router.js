      	...
      	RolesList = Roles.getRolesForUser(Meteor.userId());
	      	...
	      	if ( user && Roles.userIsInRole(user, ['admin'])){
		    	...
		    	templateData = { 
		    		...
		    		socialPosts: socialPosts.find({ $and: [{postStatus: 'new'},{postEventName: {$in: RolesList}}]}, {postDate: -1})
		    	};
		    	return templateData;
		    }
  		},
      	...
      	var user = Meteor.user();
	      	if (user && Roles.userIsInRole(user, ['admin'])){
		    	...
		    	templateData = { 
		    		//socialPosts: socialPosts.find({postStatus: this.params.type})
		    		//socialPosts: socialPosts.find({postStatus: 'deleted'}, {postDate: -1})
		    		socialPosts: socialPosts.find({ $and: [{postStatus: 'approved'},{postEventName: {$in: RolesList}}]}, {postDate: -1})
		    	};
		    	return templateData;
		    }
  		},
      	...
      	var user = Meteor.user();
	      	if (user && Roles.userIsInRole(user, ['admin'])){
		    	...
		    	templateData = { 
		    		//socialPosts: socialPosts.find({postStatus: this.params.type})
		    		//socialPosts: socialPosts.find({postStatus: 'deleted'}, {postDate: -1})
		    		socialPosts: socialPosts.find({ $and: [{postStatus: 'deleted'},{postEventName: {$in: RolesList}}]}, {postDate: -1})
		    	};
		    	return templateData;
		    }
  		},
